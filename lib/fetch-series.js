...
const SEASON_TAG_PATTERN = /^[sS]{1}-[\d]+/;
const EPISODE_TAG_PATTERN = /^[eE]{1}-[\d]+/;
		...
		return Promise.resolve(null)
			...
			.then(() => {
				...
				return client.getEventVideos(params).then(res => {
					...
					if (res.vods && Array.isArray(res.vods.data)) {
						videos = res.vods.data.map(item => {
							return item.data;
						});
					}

					if (res.live) {
						videos.unshift(res.live);
					}
	...
	function composeChildren(parent, videos) {
		...
		videos.forEach(video => {
			video.tags.forEach(tag => {
				if (SEASON_TAG_PATTERN.test(tag)) {
					tag = tagToKey(tag);
					video.season = tag;
				...
				if (EPISODE_TAG_PATTERN.test(tag)) {
					video.episode = tagToKey(tag);
					episodes.push(video);
				}
			});
		});
...
function filterNested(video) {
	...
	video.tags.forEach(tag => {
		if (EPISODE_TAG_PATTERN.test(tag.toLowerCase())) {
			episode = true;
		}
	});
