...
exports.getAllEventVideos = function (client, creds, id, maxItems) {
	...
	function fetchPage(videos, offset) {
		...
		return client.getEventVideos(params).then(res => {
			...
			// If there is a live video, push that on the list too.
			if (res.live) {
				// But don't duplicate it if it is already present in the list.
				let hasLive = false;
				videos.forEach(video => {
					if (video.id === res.live.id) {
						hasLive = true;
					}
				});
			...
			if (res.vods && Array.isArray(res.vods.data) && res.vods.data.length >= maxItems) {
